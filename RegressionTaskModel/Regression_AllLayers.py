...
with strategy.scope():
    ...
    inception_regression = keras.Model(base_model.inputs, outputs)

    inception_regression.compile(loss=tf.keras.losses.MeanAbsoluteError(), optimizer=tf.keras.optimizers.Adam(), metrics=[tf.keras.metrics.MeanAbsoluteError()]) 
    ...
    inception_regression = inception_regression.fit(train_generator,
...
#Load & Test model
filepath = path_outputs + save_name + '_singleinput.hdf5'
model_oneiput_oneoutput = load_model(filepath)
...
y_predictions=model_oneiput_oneoutput.predict_generator(test_generator,
steps=STEP_SIZE_TEST,
verbose=1)

y_predictions = list(chain.from_iterable(y_predictions.tolist()))

print('R2 score other formula: ', r_squared(test_df_filtered[variable], y_predictions))
print('Mean Absolute Error(MAE): ', sklearn.metrics.mean_absolute_error(test_df_filtered[variable], y_predictions))