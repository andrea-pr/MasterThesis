"""
Prediction of smoking variable. Undersampling majority class.
"""
import pickle
import numpy as np
...
train_df['Smoking_binarized'] = train_df['Smoking']
train_df.Smoking_binarized.replace(['current', 'ideal', 'intermediate'], [1, 0, 0], inplace=True)
val_df['Smoking_binarized'] = val_df['Smoking']
val_df.Smoking_binarized.replace(['current', 'ideal', 'intermediate'], [1, 0, 0], inplace=True)


print('Training set individuals smoking:', len(train_df[train_df['Smoking_binarized'] == 1]['eid'].unique()))
print('Training set individuals not smoking:', len(train_df[train_df['Smoking_binarized'] == 0]['eid'].unique()))

#Undersampling non smoking samples
non_smoking_ids =list(train_df[train_df['Smoking_binarized'] == 0]['eid'].unique())
undersampled_sample = resample(non_smoking_ids, replace=False, n_samples=1500, random_state=0)
undersampled_sample = undersampled_sample + list(train_df[train_df['Smoking_binarized'] == 1]['eid'].unique())
train_df = train_df[train_df['eid'].isin(undersampled_sample)]


print('Training set individuals smoking:', len(train_df[train_df['Smoking_binarized'] == 1]))
print('Training set individuals not smoking:', len(train_df[train_df['Smoking_binarized'] == 0]))
...
train_df_filtered['Smoking_binarized_string'] = train_df_filtered['Smoking_binarized']
val_df_filtered['Smoking_binarized_string'] = val_df_filtered['Smoking_binarized']

train_df_filtered.Smoking_binarized_string.replace([1, 0], ['smoking', 'non_smoking'], inplace=True)
val_df_filtered.Smoking_binarized_string.replace([1, 0], ['smoking', 'non_smoking'], inplace=True)
...
with strategy.scope():
    ...
    filepath = path_outputs + save_name + '_undersampling.hdf5'
