...
#Load Model 
filepath = '/home/andreap/fundusdata_2/results_retina_alllayers/smoking_singleinput.hdf5'
...
print('AUC All Images:', roc_auc_score(test_df_filtered['Smoking_binarized'], test_df_filtered['predictions_classes']))
...
for i in range(0, 2000): 
    ...
    true_y = resample(british_irish['Smoking_binarized'], replace=True, n_samples=sample_size, random_state=i)
...
print('AUC Other British :', roc_auc_score(other_than_british['Smoking_binarized'], other_than_british['predictions']))
...
for i in range(0, 2000): 
    ...
    true_y = resample(other_than_british['Smoking_binarized'], replace=True, n_samples=sample_size, random_state=i)
...
for i in range(0, 2000): 
    ...
    if i == 800: 
        print('800')
    predictions_classes = resample(male_df['predictions_classes'], replace=True, n_samples=sample_size, random_state=i)
    predictions = resample(male_df['predictions'], replace=True, n_samples=sample_size, random_state=i)
    true_y = resample(male_df['Smoking_binarized'], replace=True, n_samples=sample_size, random_state=i)
    auc_score_samples_male.append(roc_auc_score(true_y, predictions))

print('AUC Male', np.percentile(auc_score_samples_male, [2.5, 97.5]))

print('AUC Female:', roc_auc_score(female_df['Smoking_binarized'], female_df['predictions']))

auc_score_samples_female = []
...
for i in range(0, 2000): 
    ...
    if i == 800: 
        print('800')
    predictions_classes = resample(female_df['predictions_classes'], replace=True, n_samples=sample_size, random_state=i)
    predictions = resample(female_df['predictions'], replace=True, n_samples=sample_size, random_state=i)
    true_y = resample(female_df['Smoking_binarized'], replace=True, n_samples=sample_size, random_state=i)
    auc_score_samples_female.append(roc_auc_score(true_y, predictions))

print('AUC Female', np.percentile(auc_score_samples_female, [2.5, 97.5]))


age_between39_50 = mean_df[(mean_df['Age'] > 39) & (mean_df['Age'] <= 50)]
age_above_50 = mean_df[mean_df['Age'] > 50]

print('AUC Between 39 and 50:', roc_auc_score(age_between39_50['Smoking_binarized'], age_between39_50['predictions']))
...
for i in range(0, 2000): 
    ...
    true_y = resample(age_between39_50['Smoking_binarized'], replace=True, n_samples=sample_size, random_state=i)
...
print('AUC Above 50:', roc_auc_score(age_above_50['Smoking_binarized'], age_above_50['predictions']))
...
for i in range(0, 2000): 
    ...
    true_y = resample(age_above_50['Smoking_binarized'], replace=True, n_samples=sample_size, random_state=i)
